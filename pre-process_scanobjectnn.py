...
datapath = '../dataset/scanobjectnn/main_split_nobg/'
traindata = 'training_objectdataset_augmentedrot_scale75.h5'
testdata = 'test_objectdataset_augmentedrot_scale75.h5'
...
f = h5py.File(datapath+testdata, "r")
data = f['/data'][:]
#print(data[0].shape)
label = f['/label'][:]
#print(label)
savedic = '../dataset/scanobjectnn/projection/test/'
for i in range(data.shape[0]):
    points = data[i]
    name = classnames[label[i]] + '_' + str(i) + '.png'
    
    if not os.path.exists(savedic):
            os.makedirs(savedic)
            print("create save dic",savedic)
    savepath = savedic + name
    #print(savepath)

    img = projection.get_img(torch.from_numpy(points).unsqueeze(0))
    pic = img.detach().numpy()[0]
    ## depth pic = pic.astype(np.uint8)
    pic = pic.astype(np.uint8)*255
    #pic = pic[:, 2:-2, 2:-2]
