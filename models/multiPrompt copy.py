...
class multiPromptLearner(nn.Module):
    def __init__(self, classnames ):
        ...
        super().__init__()
        # text ptompt init
        n_ctx =4 #prompt size change transformer.py line678 as well
    ...
    def forward(self):
        ...
        return ctx,tokenized_prompts,visual_prompt,point_prompt
...
class multiPrompt(nn.Module):
    ...
    def encoder_text(self,ctx,tokenized_prompts):
        ...
        return self.clip_model.encode_text(text,ctx,tokenized_prompts)
    
    '''def get_text_prompt(self):
        return self.ctx'''
    def forward(self,pc,img,label = None): #prompt_learner
        ctx,tokenized_prompts,visual_prompt,point_prompt = self.prompt()
