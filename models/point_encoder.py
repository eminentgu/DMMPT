...
class Encoder(nn.Module):
    ...
    def forward(self, point_groups):
        #point_groups = point_groups.cuda() #?????
        '''
...
class PointcloudEncoder(nn.Module):
    def __init__(self, point_transformer, args):
        ...
        self.patch_dropout = nn.Identity()# PatchDropout(args.patch_dropout) if args.patch_dropout > 0. else nn.Identity()
        self.visual = point_transformer


    def forward(self, pts, colors,prompt):
        ...
        #print("i got prompt here: ",prompt.shape)
        #x = x + prompt
        #print("this is the pc after pointbert,add prompt here")
        #print(x.shape)
        ...
        #print(" point_deep_prompts: point_encoder line 230",len(point_deep_prompts),point_deep_prompts[0].shape)
        ###depth = len(point_deep_prompts)
        for i, blk in enumerate(self.visual.blocks):
            '''if (i>0 and i-1<depth):
                x = torch.cat((x[:,:-2,:],point_deep_prompts[i-1].unsqueeze(0).repeat(x.shape[0], 1, 1)),dim = 1)'''
            x = blk(x)
