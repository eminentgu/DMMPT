...
def default_loader(fn):
    ...
    with open(fn[1], 'r') as f:
        ...
        img = []
        for i in range(10):
            img.append(Image.open(fn[0]+str(i)+'.png').convert('RGB'))
    return (img,points)
...
class triDataset(Dataset): 
    def __init__(self,txt, transform=None,target_transform=None, loader=default_loader, small_datasets = None):
        ...
        for line in fh: 
            ...
            #print(line)
            #for i in range(1): #10
            pairs.append((('../dataset/modelnet40/modelnet40_projection/'+line[:-5]+'/'+line+'_', '../dataset/modelnet40/modelnet40_normal_resampled/'+line[:-5]+'/'+line+'.txt'), label[line[:-5]]))
        self.pairs = pairs
    ...
    def __getitem__(self, index):
        ...
        img_tensor = []
        for i in range(10):
            img_tensor.append(transf(pair[0][i]))
