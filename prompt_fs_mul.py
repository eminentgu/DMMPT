...
class ModelNetFewShot(Dataset):
    ...
    def __getitem__(self, index):
        ...
        filepath = '../dataset/modelnet40/FewShotImg/' + str(self.way) + 'Way_' + str(self.shot) + 'Shot/' + str(self.fold) + '/'+self.subset+'/'+str(index)+'_0.png'
        img = Image.open(filepath).convert('RGB')
        transf = transforms.ToTensor()
        img = transf(img)
        return current_points, label, real_label,img
...
def main(args):
    ...
    for fold in range(0,10):
        ...
        classes = {}
        for _,label,real_label, img in train_set:
        ...
        acc = 0
        best_acc = 0
        multi_prompt = multiPrompt(clip_model,model, current_class) #args ####classnames
        ...
        if evaluate:
            ...
            multi_prompt = multiPrompt(clip_model,model, current_class)#classnames
...
def train(trainData,model , optimizer, scaler, epoch, lr_schedule, args):
    ...
    for data_iter, inputs in enumerate(trainData):
        ...
        with autocast():
            ...
            label = inputs[1].cuda()'''
            pc = inputs[0].cuda(0)
            label = inputs[1].type(torch.int64).cuda(0)
            img = inputs[3].to(torch.float16).cuda(0)
            start_time = time.perf_counter()
            outputs = model(pc,label = label,img = img)
        loss = outputs['loss']
        ...
        current +=1
        loss = loss.item()
        batch_time = str(end_time-start_time)
        with open('logs.txt', 'a') as f:
