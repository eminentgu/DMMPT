...
def farthest_point_sample(xyz, npoint):
    ...
    Input:
        ...
        npoint: number of samples
    Return:
        sampled_points: sampled pointcloud, [B, npoint, 3]
    """
    device = xyz.device
    batchsize, ndataset, dimension = xyz.shape
    sampled_points = torch.zeros(batchsize, npoint, dimension, dtype=xyz.dtype).to(device)
    distance = torch.ones(batchsize, ndataset).to(device) * 1e10
    farthest = torch.randint(0, ndataset, (batchsize,), dtype=torch.long).to(device)
    batch_indices = torch.arange(batchsize, dtype=torch.long).to(device)
    
    for i in range(npoint):
        sampled_points[:, i] = xyz[batch_indices, farthest]  # 保存当前最远点的坐标
        centroid = xyz[batch_indices, farthest].view(batchsize, 1, dimension)
        dist = torch.sum((xyz - centroid) ** 2, -1)
        mask = dist < distance
        distance[mask] = dist[mask]
        farthest = torch.max(distance, -1)[1]
