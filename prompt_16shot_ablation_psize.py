...
def main(args):
    ...
    for text_prompt_size in range(2,11,2):
        torch.cuda.empty_cache()
        print("initial prompt,text prompt size:",text_prompt_size)
        best_acc = 0
        ...
        if evaluate:
            ...
            multi_prompt.load_state_dict(torch.load('text_pc_prompt_mn40_best_acc_'+str(text_prompt_size)+'.pth'), strict=False)

            #multi_prompt = torch.load('img_text_pc_prompt_mn40.pth')
            #multi_prompt.to(device_prompt)
            test_stats = evaluation(testData,multi_prompt)
            print("test on modelnet40","text_prompt_size:",text_prompt_size,"acc: ",test_stats['acc'])
            continue
        multi_prompt.prompt.cuda()
        print("prepare for training")
        epoch = 50#50
        lr = 0.001 #妈祖说微调的时候lr不能太大
        ...
        for e in range(0,epoch):
            ...
            if (e%1 == 0):
                ...
                torch.save(save_state, 'text_pc_prompt_mn40_latest_'+str(text_prompt_size)+'.pth')
            if (acc > best_acc):
                best_acc = acc
                print("saving best checkpoint,best acc:",best_acc,"epoch", e)
                torch.save(save_state, 'text_pc_prompt_mn40_best_acc_'+str(text_prompt_size)+'.pth')
